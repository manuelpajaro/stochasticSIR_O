...
%% Select a date with format year month day f= [yyyy mm dd] from 01/03/2020 to 27/06/2021
f= [2021 03 21];

%% Model Parameters 
beta  = 0.14; % beta in {0.03,0.07,0.14,0.22}
alpha = 1/14; % it can be 1/7 or 1/10 for strong decrease in infected persons (beta = 0.03)
...
simulation = SIR_ssa(alpha,beta,x0,Imax,Tgrid,nsimula);
...
%%number os realizations that are similar to the given data
...
legend('Real','SSA')
title(['SIR, ',locality,', ',num2str(f(1))],'Interpreter','latex')
set(gca,'FontSize',sizeL,'TickLabelInterpreter','latex')
